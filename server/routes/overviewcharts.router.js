...
router.get('/quarter', rejectUnauthenticated, (req, res) => {
    ...
    }else if (req.user.role_id === TEACHER && inputsValid) {
    let teacherSchool = req.user.school_id;
    ...
    JOIN "gender" ON "gender"."id" = "demographics"."gender_id"
    JOIN "race" ON "race"."id" = "demographics"."race_id" 
    WHERE ("assessments"."date" >= "school"."${quarterStart}" AND "assessments"."date" <= "school"."${quarterEnd}" AND "user"."school_id" = $1)
    GROUP BY "${filterBy}"."${searchOn}";`;
    pool.query(queryText, [teacherSchool]).then(results => {
        res.send(results.rows);
        console.log('results', results.rows);
    }).catch(error => {
        console.log('there was an error getting quarter data', error);
